...
本目录包含了 `rs-config` 项目的 CircleCI 持续集成配置。
...
**CircleCI 仪表板**：
```
https://app.circleci.com/pipelines/github/qubit-ltd/rs-config
```
...
```markdown
[![CircleCI](https://img.shields.io/circleci/build/github/qubit-ltd/rs-config/main?label=build&logo=circleci)](https://circleci.com/gh/qubit-ltd/rs-config)
```

效果预览：

![CircleCI Badge](https://img.shields.io/circleci/build/github/qubit-ltd/rs-config/main?label=build&logo=circleci)
...
```markdown
[![codecov](https://codecov.io/gh/qubit-ltd/rs-config/branch/main/graph/badge.svg)](https://codecov.io/gh/qubit-ltd/rs-config)
```
...
curl -X POST \
  ...
  https://circleci.com/api/v2/project/gh/qubit-ltd/rs-config/pipeline
```
...
- 🐛 Issue：[GitHub Issues](https://github.com/qubit-ltd/rs-config/issues)
- 💡 讨论：[GitHub Discussions](https://github.com/qubit-ltd/rs-config/discussions)
